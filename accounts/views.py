...
class LoginWithToken(APIView):
    ...
    def post(self, request):
        ...
        # If user returns NONE = wrong credentials
            ...
            response_dict={"error": "Invalid email or password"}
...
class PopularSuppliersView(APIView):
    authentication_classes = [SessionAuthentication, TokenAuthentication]
    permission_classes = [IsAuthenticated]
        
    def get(self, request):
        suppliers=Supplier.objects.filter(Q(is_popular=True))
        serializer=SupplierSerializer(suppliers, many=True)
...
class SupplierView(APIView):
    ...
    def get(self, request, supplier_id):
        try:
            ...
            response_dict=dict(error="Supplier not found")
            return Response(response_dict, status=status.HTTP_404_NOT_FOUND)
